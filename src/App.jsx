...
import { ChevronDown, Search, GraduationCap, Terminal, Github } from 'lucide-react';
    ...
    <div className="flex h-screen w-screen bg-[#0f172a] text-white font-sans overflow-hidden">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-72 bg-[#1e293b] border-r border-slate-800 transition-transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex flex-col h-full p-6">
          ...
          <nav className="flex-1 overflow-y-auto space-y-1 custom-scrollbar text-white">
          ...
          {/* Sidebar Footer (The_Bozgun & Copyright) */}
          <div className="mt-auto pt-6 border-t border-slate-800">
            <a
              href="https://github.com/BozgunBer-2506"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-3 text-slate-400 hover:text-[#FF6B35] transition-colors mb-2 group"
            >
      ...
      <main className="flex-1 overflow-y-auto bg-[#0f172a] scroll-smooth">
        {selectedSection ? (
          <article className="max-w-3xl mx-auto px-6 py-16">
            <div className="markdown-body">
              <ReactMarkdown
                ...
                components={{
                  h1: ({ children }) => <h1 className="text-4xl font-black tracking-tight mb-12 border-b border-slate-800 pb-8">{children}</h1>,
                  h2: ({ children }) => <h2 className="text-2xl font-bold text-white mt-12 mb-6 border-b border-slate-800 pb-2 tracking-tight italic">{children}</h2>,
                  ...
                  li: ({ children }) => <li className="text-slate-300 ml-6 mb-2 list-disc">{children}</li>,
                  code({ node, inline, className, children, ...props }) {
                    const match = /language-(\w+)/.exec(className || '');
                    return !inline && match ? (
                      <div className="my-6 rounded-md overflow-hidden border border-slate-800 shadow-xl">
                        <SyntaxHighlighter style={atomDark} language={match[1]} PreTag="div" customStyle={{ margin: 0, padding: '24px', fontSize: '14px' }} {...props}>
                  ...
                  div: ({ children, ...props }) => {
                    if (props.className === "answer-content") return <div className="mt-4 pt-4 border-t border-slate-700 text-slate-300 italic leading-relaxed">{children}</div>;
          ...
          <div className="h-full flex flex-col items-center justify-center p-8 text-center">
            <div className="inline-block p-4 bg-[#FF6B35]/10 rounded-3xl mb-6">
              <Terminal size={64} className="text-[#FF6B35]" />
            </div>

            <h1 className="text-7xl lg:text-8xl font-black tracking-tighter leading-none">
              Linux
            </h1>
            <h2 className="text-3xl lg:text-4xl font-bold text-[#FF6B35] tracking-tight mb-8">
              Türkçe Eğitim Rehberi
            </h2>
      ...
      <button
        ...
        className="lg:hidden fixed bottom-6 right-6 bg-[#FF6B35] text-white p-4 rounded-full shadow-2xl z-[60]"
      >
        <Search size={24} />
      </button>
    </div>
